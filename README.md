...
## WCBS V2: Package Integrity and Native Registration

WCBS V2 can install into an explicit runtime plugin directory and leaves the
user's project untouched. That proves only package integrity until the selected
runtime has a documented, executed native registration route. Every current
adapter manifest remains project-scoped, so use the safeguarded project-local
route in `GET_STARTED.md` as the default.
...
`scripts/install-adapter.mjs` is the current project-scoped default. Use the
V2 command only after a target-specific native registration route is available
