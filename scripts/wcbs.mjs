...
function assertSafePluginDir(value) {
  ...
  if (fs.existsSync(parent) && fs.lstatSync(parent).isSymbolicLink()) throw new Error("Blocked: plugin directory parent must not be a symbolic link.");
  if (fs.existsSync(absolute) && fs.lstatSync(absolute).isSymbolicLink()) throw new Error("Blocked: plugin directory must not be a symbolic link.");
  return absolute;
}
