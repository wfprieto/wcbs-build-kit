...
test("strict evaluation fails before a run directory is created when identity or loader is incomplete", () => {
  ...
  const evidenceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "wcbs-external-evidence-"));
  try {
    const runDirectory = path.join(evidenceRoot, runId);
    const result = spawnSync(process.execPath, ["scripts/run-evals.mjs", "--strict", "--execute", "--run-id", runId, "--seed", "safe-test-seed", "--evidence-dir", evidenceRoot], {
      cwd: root,
      encoding: "utf8",
...
test("execute validates an absent external evidence capability before strict preflight and never creates it in the CLI", () => {
  const runner = fs.readFileSync(path.join(root, "scripts", "run-evals.mjs"), "utf8");
  assert.match(runner, /resolveExternalEvidenceRun\(\{ root, evidence_dir: evidenceDir, run_id: runId, create: true \}\)/);
  assert.ok(runner.lastIndexOf("resolveExternalEvidenceRun") < runner.lastIndexOf("preflightProtocol"));
  assert.doesNotMatch(runner, /\bmkdir(?:Sync)?\s*\(/);
});
...
test("the runbook distinguishes blocked preflight, blinded scoring, and behavioral evidence from runtime activation", () => {
  ...
  assert.match(guide, /producer freeze/i);
  assert.match(guide, /same-authority/i);
});
