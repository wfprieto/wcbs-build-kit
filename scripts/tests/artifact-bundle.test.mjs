/**
 * Artifact-bundle validation (F-006) and exact activation-marker binding (F-010).
 *
 * HERMETIC (F-011): these tests validate a COMMITTED fixture at
 * scripts/tests/fixtures/run-bundle/, not whatever happens to sit in the
 * developer's gitignored .wcbs/ directory. The previous version assumed a live
 * run bundle existed, so a clean `git clone` + `git am` checkout failed 7 tests
 * and the advertised `npm run check` gate was not reproducible from the patch.
 *
...
test("the committed run-bundle fixture exists",()=>{assert.ok(exists(`${FIXTURE}/findings.json`));assert.ok(exists(`${FIXTURE}/progress-ledger.jsonl`));});
